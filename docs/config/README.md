...
| `interpret_text_blocks` | `false`                         | Use the command's output as the module's format string |

### Variables (not valid if interpret_text_blocks = true)
...
[custom.interpret]
command = 'echo "[red text](bold red)"' # shows output of command
when = "true"
```
